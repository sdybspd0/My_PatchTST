...
class MLP_backbone(nn.Module):
    def __init__(self, chong_len: int, jian_layers: int, nei_layers: int, c_in: int, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        # Backbone
        self.backbone = MLPEncoder(c_in, jian_layers = jian_layers, nei_layers = nei_layers, context_window = context_window, patch_num=patch_num, patch_len=patch_len, max_seq_len=max_seq_len,
        ...
        # Head
        self.head_nf = patch_len * patch_num   #已改
        self.n_vars = c_in
    ...
    def forward(self, z):  # (128,7,336)                                              # z: [bs x nvars x seq_len]
        ...
        # model
        z = self.backbone(z)  # (128,7,16,42)                                                       # z: [bs x nvars x d_model x patch_num]
...
class MLPEncoder(nn.Module):  # i means channel-independent
    def __init__(self, c_in, jian_layers: int, nei_layers: int, context_window: int, patch_num, patch_len, max_seq_len=1024,
        ...
        # Encoder能跑的
        # self.mlp_blocks = MixerBlock(self.patch_num, self.patch_len, d_model, d_ff)
    ...
    def forward(self, x) -> Tensor:  # (128,7,16,42)             # x: [bs x nvars x patch_len x patch_num]
        ...
        z = torch.reshape(x, (x.shape[0] * x.shape[1], x.shape[2], x.shape[3]))  # （896，42，16）    # u: [bs * nvars x patch_num x d_model]
        # u = self.dropout(u + self.W_pos)  # (896,42,128)                            # u: [bs * nvars x patch_num x d_model]

        # # # Encoder 能跑的
        # z = self.mlp_blocks(z) # （896，16,42）
        for block in self.mlp_blocks:
            z = block(z)   # （896，42，16）

        # Encoder
        # z = self.encoder(z)  # (896,42,128)                                               # z: [bs * nvars x patch_num x d_model]
        z = z.transpose(1,2) # （896，16,42）
        z = torch.reshape(z, ( -1, n_vars, z.shape[-2], z.shape[-1]))  # (128,7,16,42)              # z: [bs x nvars x patch_num x d_model]
        # z = z.permute(0, 1, 3, 2)  # (128,7,128,42)                                               # z: [bs x nvars x d_model x patch_num]

        return z #(128, 7, 16, 42)

class MixerBlock(nn.Module):
    def __init__(self, patch_num, patch_len, d_model, d_ff, jian_layers, nei_layers, dropout):
        super().__init__()
        # self.nei_mixing = FactorizedNeiMixing(patch_len, d_model)
        # self.jian_mixing = FactorizedJianMixing(patch_num, d_ff)
    ...
    def forward(self, src:Tensor): # （896，42，16）
        ...
        # #周期内
        # res1 = src # （896，42，16）
        # y = self.nei_mixing(src) # （896，42，16）
        #
        # y = res1 + y # （896，42，16）
        #
        # # 周期间
        # y = y.transpose(1,2) # （896，16,42）
        # res2 = y # （896，16,42）
        #
        # y = self.jian_mixing(y) # （896，16,42）
        # y = res2 + y # （896，16,42）
        # y = y.transpose(1, 2) # （896，42，16）
        #
        # return y
...
#         self.nei_mixing = MLPBlock(patch_len, d_model)
#
...
#     def __init__(self, patch_num, d_ff) :
#         super().__init__()
#
#         # assert input_dim > factorized_dim
#         self.jian_mixing = MLPBlock(patch_num, d_ff)
#
