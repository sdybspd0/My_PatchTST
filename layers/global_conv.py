...
class GConv(nn.Module):
    ...
    def forward(self, u, return_kernel=False): #（32，336，7）
        """
        ...
        if 'sum' in self.mode:
            ...
            for i in range(self.num_scales):
                kernel = F.interpolate(
                    ...
                    mode=interpolate_mode,
                ) * multiplier ** (self.num_scales - i - 1)  #(32,7,32) (32,7,32) (32,7,64) (32,7,128) (32,7,256)
                kernel_list.append(kernel)
            k = torch.cat(kernel_list, dim=-1)  #(32,7,512)
        else:
        ...
        if not self.kernel_norm_initialized:
            self.kernel_norm = k.norm(dim=-1, keepdim=True).detach() #(32,7,1)
        ...
        # Convolution
        if self.bidirectional:
            k0, k1 = rearrange(k, '(s c) h l -> s c h l', s=2)  #(16,7,336) (16,7,336)
            k = F.pad(k0, (0, L)) + F.pad(k1.flip(-1), (L, 0))   #(16,7,672)
        
        
        k_f = torch.fft.rfft(k, n=2*L)  # (C H L) (16,7,337)
        u_f = torch.fft.rfft(u, n=2*L)  # (B H L) (32,7,337)
        # k_f.unsqueeze(-4) * u_f.unsqueeze(-3) # (B C H L)
        
        y_f = contract('bhl,chl->bchl', u_f, k_f) #(32,16,7,337)
        y = torch.fft.irfft(y_f, n=2*L)[..., :L]  # (B C H L) (32,16,7,336)

        # Compute D term in state space equation - essentially a skip connection
        y = y + contract('bhl,ch->bchl', u, self.D) # (32,16,7,336)

        # Reshape to flatten channels
        y = rearrange(y, '... c h l -> ... (c h) l') # (32,112,336)

        if not self.linear:
            y = self.dropout(self.activation(y)) # (32,112,336)

        if not self.transposed:
            y = y.transpose(-1, -2) #(32,336,112)

        if not self.linear:
            y = self.norm(y)
            y = self.output_linear(y) #(32,336,7)
