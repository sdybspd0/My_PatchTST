...
class Exp_Main(Exp_Basic):
    ...
    def train(self, setting):
        train_data, train_loader = self._get_data(flag='train') #(34129,1066)
        vali_data, vali_loader = self._get_data(flag='val') #(11185,349)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                batch_x = batch_x.float().to(self.device) #(32,336,7)
                batch_y = batch_y.float().to(self.device) #(32,144,7)
                batch_x, batch_y = self.get_random_channel(batch_x, batch_y)  #打乱通道顺序

                if self.args.perturb_ratio != 0:  #为数据添加随机噪声
                ...
                # decoder input
                dec_inp = torch.zeros_like(batch_y[:, -self.args.pred_len:, :]).float() #(32,96,7)
